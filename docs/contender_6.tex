...
\subsection{Ordinal arithmetic in four \texttt{Fixpoint}s}

The natural numbers embed as iterated successors, and $\omega$ is
their limit:
...
Addition, multiplication, and $\omega$-exponentiation follow the
textbook recursion equations, in each case by structural recursion
with limits pushed through pointwise:
...
The enumerator gains \texttt{tGrow} as one more constant segment:
...
budget is listed -- so only that direction is ported (the proof is the
same induction as upstream's, with one extra trivial case for
\texttt{tGrow}):
...
results.  The relation lifts to packs -- \emph{same} type tag,
related values -- and pointwise to environments:
...
(induction on the type).  The second says look-up in related
environments misses on both sides or hits related packs on both sides
-- \texttt{Forall2} forces equal lengths, so the reverse-level
index arithmetic agrees.  The third is the interesting one: it is
proved simultaneously with its own converse, because casting a
\emph{function} casts its argument \emph{backward} -- precisely the
usual contravariance twist, and the reason the two directions cannot
be separated.
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{\texttt{tVar}}: both sides look up the same index;
        ...
        miss-miss (default packs, related by \texttt{error\_related})
        and refutes the mixed cases.
  \item \textbf{\texttt{tLam}}: both sides build a function pack at
        tag $\texttt{tpArr}\,A\,B$; given related arguments, the
        extended environments are related, the induction hypothesis
        relates the interpreted bodies, and \texttt{cast\_related}
...
For closed terms the empty environments are trivially related, so the
fundamental theorem yields a \emph{common} tag $tp$ and related values
$v_1, v_2$.  If $tp = \texttt{tpNat}$, then $v_1 = v_2$ and the two
...
completes in about five seconds and \texttt{coqchk} independently
...
author's fork; the contender-6 section occupies lines $830$--$1380$
