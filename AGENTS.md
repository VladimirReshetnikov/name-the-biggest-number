...
(Updated 2026-07-02, repository consolidation session.)
...
- `submit-c6-prep` — `ideas` plus the in-progress contender_6
  ...
  *unconditionally* (on arrow-typed embeddings both sides collapse
  ...
  switch to TeX Gyre Pagella + Monaspace).  The branch is kept as a
  strict superset of `ideas`; merge `ideas` forward into it from time
  to time.  Build artifacts of `contender_6.tex`
  (`docs/contender_6.{pdf,log,out,toc}`, `texput.log`) are
  git-ignored — only sources are committed.  Regenerate the PDF with
  `lualatex -interaction=nonstopmode -output-directory=docs
  docs/contender_6.tex` (run twice for the table of contents).
- Deleted branches (2026-07-02 consolidation; all history is preserved
  in `ideas`):
  - `tooling` (minimal Coq build/audit scripts, cleanliness check,
    status dashboard) was merged into `ideas` at `ce1ba72` and the
    branch pointer deleted.
  - `manual` ("Convert Coq 8.19 reference manual to Markdown":
    `docs/coq-8.19.0-reference-manual.md` generated by
    `scripts/convert_coq_refman_to_md.py`) lived only in a temporary
    second clone at `C:\repos\name-the-biggest-number-2` (created
    2026-05-01).  It was fetched into this repository, merged into
    `ideas` at `c2bb154`, and both the branch pointer and the entire
    second clone were then deleted.  Everything now lives in the
    single working directory `C:\repos\name-the-biggest-number`.
  - `claude/dazzling-heyrovsky-ecf14a` was an auto-created Claude Code
    session worktree branch (under `.claude/worktrees/`) whose only
    session made zero repo changes; the worktree and branch were
    removed.
- Branches named `claude/*` with worktrees under `.claude/worktrees/`
  are transient Claude Code session artifacts.  When such a branch has
  a clean tree and no unmerged commits, it is safe to
  `git worktree remove` the worktree and delete the branch.
...
### RocqIDE on this machine

- `rocqide.exe` needs the environment variable `GDK_IGNORE_WINTAB=1`
  (already persisted at User scope on this box, verified 2026-07-02).
  Without it, GDK hangs during delayed Wintab (tablet driver)
  initialization against a stale `C:\Windows\system32\Wintab32.dll`
  and the GUI window never appears.  The empty console window that
  accompanies RocqIDE is expected — `rocqide.exe` is a
  console-subsystem binary.
- Known RocqIDE 9.0.1 bug (diagnosed locally 2026-05-06, not yet
  reported upstream): entering `Check Type@{Top.56}.` freezes the IDE.
  The lexer error itself is handled fine by `coqidetop`; the hang is in
  the IDE's error-range highlighting, which loops on the GUI thread
  inside GTK text-tag machinery (`ide/rocqide/coqOps.ml` apply-tag
  path).  No preference exists to disable error-range highlighting;
  use `coqtop` / `rocq repl` for experimental input instead.
